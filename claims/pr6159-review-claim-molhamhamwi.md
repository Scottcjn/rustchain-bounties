...
- Ran focused regression tests with a Python version compatible with the repository syntax (`>=3.10` is needed for the existing `str | None` annotations):

  ```text
  uv run --python 3.13 pytest tests/test_signed_transfer_replay.py -q
  10 passed
  ```

## Notes

No blockers found. I also tried the host default `python3 -m pytest tests/test_signed_transfer_replay.py -q`; that Python is 3.9 and failed during collection on existing `str | None` type-union syntax, so the validation was rerun with a compatible interpreter.
