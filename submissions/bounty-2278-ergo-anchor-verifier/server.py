...
<div class="container">
  <header>
    <div>
      ...
      <p style="color:var(--muted);font-size:0.9rem">
        RustChain Independent Audit Tool | Bounty #2278
      </p>
    </div>
    <div id="ergo-status">
      <span class="badge badge-offline">Checking Ergo Node...</span>
    </div>
  </header>
  ...
  <div class="stats" id="stats">
    ...
    <div class="stat-card">
      ...
      <div class="value match" id="matched">--</div>
    </div>
    <div class="stat-card">
      <div class="label">Mismatches</div>
      <div class="value mismatch" id="mismatched">--</div>
    </div>
    <div class="stat-card">
      <div class="label">Errors</div>
      <div class="value error" id="errors">--</div>
    </div>
    <div class="stat-card">
      <div class="label">Integrity Score</div>
      <div class="value" id="integrity" style="color:var(--accent)">--</div>
      <div class="progress-bar"><div class="progress-fill" id="integrity-bar"
  ...
  <table id="results-table">
    <thead>
      <tr>
        <th>Anchor</th><th>Epoch</th><th>Ergo TX</th>
        <th>Miners</th><th>Status</th><th>Reason</th>
      </tr>
    </thead>
    <tbody id="results-body">
      <tr><td colspan="6" style="text-align:center;color:var(--muted)">
        Click "Run Full Verification" to begin audit
      </td></tr>
    </tbody>
  </table>
  ...
  <div class="detail-panel" id="detail-panel">
    ...
    <div class="detail-row">
      ...
      <span id="detail-epoch" class="mono"></span>
    </div>
    <div class="detail-row">
      <span class="detail-label">Ergo Transaction</span>
      <span id="detail-tx" class="mono"></span>
    </div>
    <div class="detail-row">
      <span class="detail-label">Miner Count</span>
      <span id="detail-miners"></span>
    </div>
    <div class="detail-row">
    ...
    <div class="commit-compare">
      <div class="commit-box" id="commit-stored">
        <div class="commit-label">Stored (DB)</div>
        <div id="commit-stored-val">--</div>
      </div>
      <div class="commit-box" id="commit-onchain">
        <div class="commit-label">On-Chain (R4)</div>
        <div id="commit-onchain-val">--</div>
      </div>
      <div class="commit-box" id="commit-recomp">
        <div class="commit-label">Recomputed</div>
        <div id="commit-recomp-val">--</div>
      </div>
    </div>
  </div>

  <footer>
    Ergo Anchor Chain Proof Verifier &mdash; RustChain Bounty #2278 (100 RTC)
    &mdash; ElromEvedElElyon
  </footer>
</div>
...
async function runVerification() {
  ...
  try {
    ...
    document.getElementById('run-status').textContent =
      `Completed in ${data.duration_sec}s (Run #${data.run_id})`;
  } catch (e) {
...
function renderTable(results) {
  ...
  body.innerHTML = '';
  if (!filtered.length) {
    body.innerHTML = '<tr><td colspan="6" style="text-align:center;color:var(--muted)">No results</td></tr>';
    return;
  }
  filtered.forEach(r => {
    ...
    tr.innerHTML = `
      <td class="mono">#${r.anchor_id}</td>
      <td>${r.epoch}</td>
      <td class="mono truncate" title="${r.ergo_tx_id}">${(r.ergo_tx_id||'').slice(0,12)}...</td>
      <td>${r.miner_count || '--'}</td>
      <td><span class="status-pill status-${r.status}">${r.status}</span></td>
      <td style="color:var(--muted);font-size:0.85rem">${r.reason || ''}</td>
    `;
    body.appendChild(tr);
  });
}
...
function showDetail(r) {
  ...
  const stored = document.getElementById('commit-stored');
  const onchain = document.getElementById('commit-onchain');
  const recomp = document.getElementById('commit-recomp');
  ...
  stored.className = 'commit-box ' + (r.stored_commit === r.recomp_commit ? 'match-yes' : 'match-no');
  onchain.className = 'commit-box ' + (isMatch ? 'match-yes' : 'match-no');
  recomp.className = 'commit-box match-yes';
}
...
fetch('/api/status').then(r => r.json()).then(d => {
  ...
  if (d.ergo_online) {
    el.innerHTML = '<span class="badge badge-online">Ergo Node Online</span>';
  } else {
    el.innerHTML = '<span class="badge badge-offline">Offline Mode (Simulated)</span>';
  }
}).catch(() => {});
</script>
</body>
...
async def handle_status(request: web.Request) -> web.Response:
    """Return system status including Ergo node connectivity."""
    app = request.app
    ...
    return web.json_response({
        "ergo_online": ergo.online,
        "ergo_node": ERGO_NODE,
        "db_path": DB_PATH,
        "anchor_count": anchor_count,
        "attestation_count": attest_count,
        "verification_runs": run_count,
...
async def handle_verify(request: web.Request) -> web.Response:
    ...
    app = request.app
    result = await run_full_verification(app["db"], app["ergo"])
    return web.json_response(result)


async def handle_results(request: web.Request) -> web.Response:
    """Return latest verification results from the database."""
    ...
    if row is None:
        return web.json_response({"results": [], "summary": {
    ...
    results = []
    async with db.execute(
        """SELECT vr.anchor_id, vr.epoch, ea.ergo_tx_id,
                  vr.stored_commit, vr.onchain_commit, vr.recomp_commit,
                  vr.status, vr.reason
           FROM verification_results vr
           JOIN ergo_anchors ea ON ea.anchor_id = vr.anchor_id
           WHERE vr.verified_at >= (
        ...
        async for r in cur:
            results.append({
                "anchor_id": r[0], "epoch": r[1], "ergo_tx_id": r[2],
                "stored_commit": r[3], "onchain_commit": r[4],
                "recomp_commit": r[5], "status": r[6], "reason": r[7],
                "miner_count": None,
            })
...
async def handle_anchors(request: web.Request) -> web.Response:
    """List all anchors from the database."""
    db: aiosqlite.Connection = request.app["db"]
    anchors = []
    async with db.execute(
        "SELECT anchor_id, epoch, ergo_tx_id, commitment, box_id, anchor_ts, "
        "status, block_height FROM ergo_anchors ORDER BY epoch"
    ) as cur:
        async for row in cur:
            anchors.append({
                "anchor_id": row[0], "epoch": row[1], "ergo_tx_id": row[2],
                "commitment": row[3], "box_id": row[4],
                "anchor_ts": row[5], "status": row[6],
                "block_height": row[7],
            })
...
async def handle_anchor_detail(request: web.Request) -> web.Response:
    """Get detailed info for a specific anchor including miner data."""
    anchor_id = int(request.match_info["anchor_id"])
    db: aiosqlite.Connection = request.app["db"]

    async with db.execute(
        "SELECT anchor_id, epoch, ergo_tx_id, commitment, box_id, anchor_ts, "
        "status, block_height FROM ergo_anchors WHERE anchor_id = ?",
        (anchor_id,),
    ) as cur:
        row = await cur.fetchone()
    ...
    miners = []
    async with db.execute(
        "SELECT miner_id, architecture, timestamp, hw_fingerprint, "
        "trust_score, rtc_earned FROM miner_attest_recent WHERE epoch = ?",
        (anchor["epoch"],),
    ) as cur:
        async for m in cur:
            miners.append({
                "miner_id": m[0], "architecture": m[1], "timestamp": m[2],
...
async def handle_epochs(request: web.Request) -> web.Response:
    """Return epoch summary statistics."""
    db: aiosqlite.Connection = request.app["db"]
    epochs = []
    async with db.execute(
        """SELECT epoch, COUNT(*) as miners,
                  SUM(rtc_earned) as total_rtc,
                  AVG(trust_score) as avg_trust,
                  GROUP_CONCAT(DISTINCT architecture) as archs
           FROM miner_attest_recent
           GROUP BY epoch ORDER BY epoch"""
    ) as cur:
        async for row in cur:
            epochs.append({
                "epoch": row[0], "miners": row[1],
                "total_rtc": round(row[2], 2),
                "avg_trust": round(row[3], 4),
                "architectures": row[4].split(",") if row[4] else [],
            })
