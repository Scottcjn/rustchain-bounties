...
def miner_status_str(miner_id: str) -> str:
...
async def cmd_balance(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    msg = await update.message.reply_text(f"🔍 Querying `{wallet_id}`…", parse_mode="Markdown")
    data = get_balance(wallet_id)
    if "error" in data:
...
async def cmd_miner(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    msg = await update.message.reply_text(f"🔍 Checking `{miner_id}`…", parse_mode="Markdown")
    status = miner_status_str(miner_id)
    data = get_balance(miner_id)
    balance = data.get("balance", data.get("rtc_balance", "N/A"))
    text = (
        f"⛏️ *Miner:* `{miner_id}`\n"
        f"*Status:* {status}\n"
...
async def cmd_epoch(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        f"*Epoch:* `{epoch_num}`\n"
        f"*Slot:* `{slot}`\n"
        f"*Enrolled Miners:* `{miners_count}`\n"
        f"*Next Settlement:* `{next_s}s`",
        parse_mode="Markdown",
    )
...
async def cmd_health(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    data = get_health()
    if "error" in data:
        await update.message.reply_text(f"❌ Node unreachable: {data['error']}")
        return
