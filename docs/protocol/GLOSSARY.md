...
| **Fleet Detection** | A system that identifies coordinated mining operations using IP similarity, hardware fingerprint similarity, and timing patterns. Miners flagged as a fleet suffer reward decay. |
| **Fleet Score** | A value between 0 and 1 calculated from three signal vectors. Scores above 0.45 trigger fleet flagging; above 0.6 trigger reward decay. |
| **False Positive** | A scenario where genuinely independent miners are incorrectly flagged as a coordinated fleet. |
...
| **RIP-201** | RustChain Improvement Proposal #201, the fleet detection immune system deployed 2026-02-28 to prevent coordinated mining farms. |
