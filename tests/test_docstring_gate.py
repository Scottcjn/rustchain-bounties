...
class WeeklyCapIdempotencyTests(unittest.TestCase):
    def setUp(self):
        self._gh = dg.gh
        self._add_labels = dg.add_labels
        self._gh_raw = dg.gh_raw
        self._docstring_rtc_this_week = dg.docstring_rtc_this_week

    def tearDown(self):
        dg.gh = self._gh
        dg.add_labels = self._add_labels
        dg.gh_raw = self._gh_raw
        dg.docstring_rtc_this_week = self._docstring_rtc_this_week
    ...
    def test_weekly_cap_reached_label_skips_duplicate_adjudication(self):
        ...
        def fake_gh(args, default=None, strict=False):
            if args[:2] == ["issue", "view"]:
                return {
                    ...
                    "body": "PR: https://github.com/Scottcjn/Rustchain/pull/8305",
                    "labels": [{"name": "weekly-cap-reached"}],
                    "author": {"login": "someone"},
                    "state": "OPEN",
                }
            if args[:3] == ["issue", "comment", dg.NUM]:
        ...
        dg.gh_raw = lambda args: (_ for _ in ()).throw(AssertionError(f"unexpected gh_raw call: {args}"))
        dg.docstring_rtc_this_week = lambda author: (_ for _ in ()).throw(AssertionError("should not query weekly earnings"))
        old_num = dg.NUM
        try:
            dg.NUM = "16711"
