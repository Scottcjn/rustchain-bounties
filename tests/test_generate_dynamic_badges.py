...
from generate_dynamic_badges import (
    ...
    color_for_level,
    slugify_hunter,
    slugify_hunter_collision_safe,
    write_badge,
    validate_badge_schema,
    get_category_hunters,
    CATEGORY_BADGES,
)
...
class TestSlugifyHunterCollisionSafe:
    """Test slugify_hunter_collision_safe function - edge cases for v2"""

    def test_no_collision_returns_base(self):
        used = set()
        result = slugify_hunter_collision_safe("@hunter1", used)
        assert result == "hunter1"
        assert "hunter1" in used

    def test_collision_adds_suffix(self):
        used = {"hunter1"}
        result = slugify_hunter_collision_safe("@hunter1", used)
        assert result == "hunter1-1"

    def test_multiple_collisions_increment(self):
        used = {"hunter1", "hunter1-1"}
        result = slugify_hunter_collision_safe("@hunter1", used)
        assert result == "hunter1-2"

    def test_empty_after_strip(self):
        used = set()
        result = slugify_hunter_collision_safe("@@@", used)
        assert result == "unknown"


class TestValidateBadgeSchema:
    """Test validate_badge_schema function - edge cases"""

    def test_valid_badge_passes(self):
        payload = {
            "schemaVersion": 1,
            "label": "Test",
            "message": "value",
        ...
        # Should not raise
        validate_badge_schema(payload, Path("test.json"))
    ...
    def test_missing_schema_version_raises(self):
        ...
        with pytest.raises(ValueError, match="Missing required field: schemaVersion"):
            validate_badge_schema(payload, Path("test.json"))

    def test_invalid_schema_version_type_raises(self):
        payload = {
            "schemaVersion": "1",  # Should be int
            "label": "Test",
            "message": "value",
            "color": "blue",
        }
...
class TestGetCategoryHunters:
    """Test get_category_hunters function - edge cases for category badges"""

    def test_empty_rows(self):
        result = get_category_hunters([])
        assert result == {"docs": [], "bug": [], "outreach": []}

    def test_docs_badge_detected(self):
        rows = [
            {"hunter": "@docwriter", "xp": 100, "level": 1, "title": "Starting", 
             "badges": "![Tutorial Titan](url)"},
        ]
        result = get_category_hunters(rows)
        assert len(result["docs"]) == 1

    def test_bug_slayer_detected(self):
        rows = [
            {"hunter": "@bugfixer", "xp": 200, "level": 2, "title": "Basic",
             "badges": "![Bug Slayer](url)"},
        ]
        result = get_category_hunters(rows)
    ...
    def test_case_insensitive_detection(self):
        rows = [
            {"hunter": "@tester", "xp": 50, "level": 1, "title": "Starting",
             "badges": "TUTORIAL TITAN"},
        ]
        result = get_category_hunters(rows)
        assert len(result["docs"]) == 1

    def test_no_category_badges(self):
        rows = [
            {"hunter": "@newbie", "xp": 50, "level": 1, "title": "Starting",
             "badges": "First Blood"},
        ]
        result = get_category_hunters(rows)
...
class TestParseRowsWithBadges:
    """Test parse_rows now extracts badges column"""

    def test_parses_badges_column(self):
        md = """# XP Tracker

| Rank | Hunter | Wallet | XP | Level | Title | Badges | Last Action | Notes |
|------|--------|--------|-----|-------|-------|--------|-------------|-------|
| 1 | @hunter1 | wallet1 | 1000 | 4 | Rising Hunter | First Blood | 2026-02-01 | - |
"""
        rows = parse_rows(md)
        assert len(rows) == 1
        assert rows[0]["badges"] == "First Blood"

    def test_parses_last_action_column(self):
        md = """# XP Tracker

| Rank | Hunter | Wallet | XP | Level | Title | Badges | Last Action | Notes |
|------|--------|--------|-----|-------|-------|--------|-------------|-------|
| 1 | @hunter1 | wallet1 | 1000 | 4 | Rising Hunter | - | 2026-02-18 (+300 XP) | - |
"""
        rows = parse_rows(md)
        assert len(rows) == 1
        assert "300 XP" in rows[0]["last_action"]
