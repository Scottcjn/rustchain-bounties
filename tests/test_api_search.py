...
class TestApiSearch(unittest.TestCase):
    ...
    def test_search_invalid_page(self):
        response = self.app.get("/api/search?page=abc")
        self.assertEqual(response.status_code, 400)
        self.assertIn("Invalid page parameter", response.json["error"])

    def test_search_out_of_range_page(self):
        response = self.app.get("/api/search?page=0")
        self.assertEqual(response.status_code, 400)
        self.assertIn("page parameter out of range", response.json["error"])

    def test_search_invalid_per_page(self):
        response = self.app.get("/api/search?per_page=abc")
        self.assertEqual(response.status_code, 400)
        self.assertIn("Invalid per_page parameter", response.json["error"])

    def test_search_out_of_range_per_page(self):
        response = self.app.get("/api/search?per_page=100")
        self.assertEqual(response.status_code, 400)
