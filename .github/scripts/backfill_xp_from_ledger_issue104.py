...
def main() -> None:
    ...
    if Path(args.comments_json).exists():
        ...
        print(
            f"Found {len([e for e in all_entries if e.source.startswith('comment')])} entries from comments"
        )
    ...
    if args.dry_run:
        print("\nDry run - no XP applied")
        return

    # Apply XP using update_xp_tracker_api.py
    ...
    for user, total in user_totals.items():
        ...
        cmd = [
            ...
            tier,
            "--reason",
            "backfill from issue #104 ledger",
        ]

        print(f"Applying {tier} XP to {user}...")
        result = subprocess.run(cmd, capture_output=True, text=True)
        if result.returncode != 0:
            print(f"Error applying XP to {user}: {result.stderr}")
        else:
