...
def main() -> None:
    ...
    if Path(args.comments_json).exists():
        ...
        for i, comment in enumerate(comments_data):
            source = f"comment-{i}"
            entries = parse_comment_entries(comment.get("body", ""), source)
            comment_entries.extend(entries)

        all_entries.extend(comment_entries)
        print(f"Found {len(comment_entries)} entries from comments")
    ...
    for entry in valid_entries:
        user_totals[entry.user] = user_totals.get(entry.user, 0.0) + entry.amount

    # Apply XP for each user
    for user, total_amount in user_totals.items():
        tier = tier_for_amount(total_amount)
        print(f"{user}: {total_amount} RTC -> {tier} tier")
