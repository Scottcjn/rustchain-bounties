...
#   - trailing annotation (`- please send RTC`)
WALLET_HANDLE_RE=re.compile(r'(?i)(?:##?\s*)?(?:-?\s*)?(?:\*\*)?Wallet\s*:\s*([a-zA-Z0-9_-]+)(?:\s*\(?GitHub handle\)?|\s*- please send RTC)?')

def get_wallet_handle(issue_body):
    """Extracts the wallet handle from the issue body."""
    match = WALLET_HANDLE_RE.search(issue_body)
    return match.group(1) if match else None

def get_wallet_address(issue_body):
    """Extracts the wallet address from the issue body."""
    match = WALLET_RE.search(issue_body)
    return match.group(0) if match else None

def pay_bounty(wallet, amount):
    """Pays the bounty to the specified wallet."""
    # Implement the payment logic here
    pass

def main():
    # Get the list of verified-eligible claims
    claims = []
    # Implement the logic to fetch the claims here
    for claim in claims:
        issue_body = claim["issue_body"]
        wallet_handle = get_wallet_handle(issue_body)
        wallet_address = get_wallet_address(issue_body)
        if wallet_address:
