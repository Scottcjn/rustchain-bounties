...
function toolResult(content: string): { content: Array<{ type: string; text: string }> } {
...
const RUSTCHAIN_BOUNTIES_URL =
  "https://raw.githubusercontent.com/Scottcjn/rustchain-bounties/main/BOUNTY_LEDGER.md";

async function fetchBounties(): Promise<string> {
  try {
    const res = await fetch(RUSTCHAIN_BOUNTIES_URL, { signal: AbortSignal.timeout(10_000) });
    if (!res.ok) return `Could not fetch bounty ledger (HTTP ${res.status})`;
    const text = await res.text();

    // Parse markdown table: | # | Title | Reward | Status |
    const lines = text.split("\n");
    const rows: string[] = ["## Open RustChain Bounties", ""];
    ...
    tools: [
      {
        ...
        description:
          "Check RustChain node health across all known nodes with automatic failover. " +
      ...
      },
      {
        ...
        description:
          "Query the RTC balance for a wallet or miner ID on the RustChain network. " +
      ...
      },
      {
        ...
        description:
          "List all active miners currently participating in the RustChain network, " +
      ...
      },
      {
        ...
        description:
          "Get the current epoch information including epoch number, slot, block height, " +
      ...
      },
      {
        ...
        description:
          "Submit a hardware fingerprint attestation for a miner to join the RustChain network. " +
      ...
      },
      {
        ...
        description:
          "List all open RustChain bounties from the official bounty ledger, " +
